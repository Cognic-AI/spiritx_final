...
def main(user_query, custom_domains,tags,location,request_id):
    print("user_query")
    print(user_query)
    ...
    result = entrypoint_agent.initiate_chats([
        {
            ...
            "message": f"Please execute the `generate_search_results` function. The user query is: {user_query}. Domains to prioritize: {custom_domains}. Tags are: {tags}. Request ID: {request_id}",
            "max_turns": 2,
            "summary_method": "last_msg",
        },
        {
            "recipient": product_selection_agent,
            "message": f"Please execute the `extract_all_links` function. product or item name is {user_query}. Domains to prioritize: {custom_domains}. location is {location}. Request ID: {request_id}",
            "max_turns": 2,
            "summary_method": "last_msg",
        },
        {
            "recipient": data_extract_agent,
            "message": f"Please execute the `process_links` function. Domains to prioritize: {custom_domains}. location is {location}. Request ID: {request_id}", 
            "max_turns": 2,
            "summary_method": "last_msg",
        },
        {
...
# result = main("White Sauce",None,["white","Sauce","Quality"],"1234567890")